...
class ConfigLoader:
    ...
    def __init__(self, config_path=None):
        if config_path is None:
            # 1. Ubicación de este archivo (.../core/config_loader.py)
            current_dir = os.path.dirname(os.path.abspath(__file__))
            
            # 2. Subimos a la raíz del proyecto
            project_root = os.path.dirname(current_dir)
            
            # 3. ¡AQUÍ ESTÁ EL TRUCO! Debemos entrar a la carpeta 'config'
            # Asegúrate de que diga 'config' ANTES de 'config.ini'
            config_path = os.path.join(project_root, 'config', 'config.ini')

        if not os.path.exists(config_path):
            raise FileNotFoundError(
                f"\n[iBot Error] No se encontró el archivo en la ruta calculada:\n"
                f"--> {os.path.abspath(config_path)}\n"
                f"Asegúrate de que 'config.ini' esté DENTRO de la carpeta 'config'."
            )
